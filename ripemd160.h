//	Copyright© 2022 AncientTides
//  IN THE EVENT THAT PERMISSION AND AUTHORIZATION IS GIVEN, ISSUED OR GRANTED BY RIGHTFUL OWNER - MR DAVID GRADASUPREME AI,
//	THE SOFTWARE IS PROVIDED "AS IS", WITHOUT WARRANTY OF ANY KIND, EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED
//	TO THE WARRANTIES OF MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE AND NONINFRINGEMENT. IN NO EVENT SHALL THE 
//  AUTHOR(S) OR COPYRIGHT HOLDER(S) BE LIABLE FOR ANY CLAIM, DAMAGES OR OTHER LIABILITY, WHETHER IN AN ACTION OF CONTRACT,
//	TORT OR OTHERWISE, ARISING FROM, OUT OF OR IN CONNECTION WITH THE SOFTWARE OR THE USE, MISUSE OF OR OTHER DEALINGS IN THE SOFTWARE.
...
string uint8_to_hex(const uint8_t *buffer, size_t hash_length);
...
void Initialize_ripemd160(const uint8_t* msg, uint32_t msg_len, uint8_t* hash)
{
...
string ripemd160(string inputData)
{
	size_t input_length = inputData.length();
	uint8_t data[input_length];
	size_t hash_length = 20;
	uint8_t hash_buffer[hash_length];
	memcpy(data, inputData.c_str(), sizeof(data));
	Initialize_ripemd160(data, input_length, hash_buffer);
	string convertByte2Hex = uint8_to_hex(hash_buffer, hash_length);
	return convertByte2Hex;
} // SUCCESSFUL DEBUG ON 11/05/2022 10:24PM
...
string uint8_to_hex(const uint8_t *buffer, size_t hash_length)
{
	...
	for (int i = 0; i < hash_length; i++)
		op << hex << setw(2) << static_cast<int>(buffer[i]);
	return op.str();
}
